...
9. **Asking questions**: Ask clarification or user feedback questions by placing them in .md files, or for 1-3 simple questions, use the AskUserQuestion tool. For larger question sets (4+), use the .md file approach.

### Claude Code Tool Note: Context Management

When loading artifacts for session continuity, optimize tool usage:

- **Same session (continuing work)**: Artifacts already in context do not need to be re-read. Only read files that have changed or were not previously loaded.
- **New session (returning user)**: Use Glob to discover available artifacts before reading them:
  - `aidlc-docs/inception/**/*.md` -- Inception phase artifacts
  - `aidlc-docs/construction/**/*.md` -- Construction phase artifacts
  - `aidlc-docs/aidlc-state.md` -- Always read first to determine current stage
- **Bulk loading**: When multiple artifacts are needed, issue parallel Read calls rather than sequential reads to minimize round-trips.
- **Selective loading**: Follow the "Smart Context Loading by Stage" guidelines (item 4 above) to avoid loading unnecessary artifacts.

...
### Claude Code Alternative: AskUserQuestion Tool

**Note for Claude Code Plugin**: You MAY use the `AskUserQuestion` tool as a lighter-weight alternative to creating a dedicated question file in certain scenarios. Use the decision matrix below to choose the appropriate method.

#### Decision Matrix: AskUserQuestion vs Question File

| Criteria | AskUserQuestion Tool | Question File (.md) |
|---|---|---|
| Number of questions | 1-3 questions | 4+ questions |
| Question complexity | Simple, single-choice | Multi-part, needs explanation |
| Options per question | 2-4 options | 2-6 options + "Other" |
| Contradiction detection | Not needed | Required (see below) |
| User response tracking | Transient (session only) | Persistent (saved to file) |
| Best for | Initial requirements, confirmations, quick decisions | Detailed analysis, classification, design choices |

**When in doubt**: If questions require "Other" options with free-text descriptions, or if responses need contradiction/ambiguity detection, use the question file approach.

For larger question sets (4+ questions), always use the question file approach described below.

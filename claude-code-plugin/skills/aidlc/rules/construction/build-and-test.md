...
### Claude Code Tool Note: Build and Test Execution

When executing build and test commands, use the Bash tool with these patterns:

**Dependency installation**:
```bash
# Run with appropriate timeout for large dependency trees
npm install          # Node.js
...
**Build execution**:
```bash
# Chain build commands; stop on first failure
...
**Test execution**:
```bash
# Run tests with output captured for analysis
npm test 2>&1
mvn test 2>&1
pytest tests/ -v 2>&1
go test ./... 2>&1
...
1. Run build/test command via Bash and capture output
2. If errors occur, use Grep to locate the failing source lines
3. Use Read to examine the relevant file context
4. Use Edit to apply the fix
5. Re-run the build/test command to verify the fix

**Parallel test execution**: When the project has independent test suites (e.g., unit tests and linting), run them as separate parallel Bash calls to reduce total execution time.

...
- [ ] Generate exactly what the current step describes:
  - **If file exists**: Modify it in-place (never create `ClassName_modified.java`, `ClassName_new.java`, etc.)
    - *Claude Code*: Use the `Edit` tool for in-place modifications; use `Write` only for new files.
...
### Claude Code Tool Note: Edit vs Write

When generating or modifying code files, choose the appropriate tool:

| Scenario | Tool | Reason |
|---|---|---|
| New file (greenfield) | Write | Creates file from scratch |
| New file (brownfield, file does not exist) | Write | No existing content to preserve |
| Modify existing file | Edit | Preserves unchanged code, targets specific sections |
| Replace entire file content | Write | Only when full rewrite is intended and approved |
| Small targeted change | Edit | Minimizes risk of unintended changes |

- **Always Read before Edit**: Read the target file first to understand its current content and identify the exact text to replace.
- **Prefer Edit for brownfield**: Edit produces smaller, reviewable diffs compared to full file rewrites with Write.
